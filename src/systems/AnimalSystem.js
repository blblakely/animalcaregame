...
export class AnimalSystem {
  ...
  create(species, enclosure) {
    ...
    const animal = { id:this.nextId++, name:this.pick(NAMES), species, age:1 + Math.floor(this.random()*9), traits:[first,second], hunger:65+Math.floor(this.random()*25), cleanliness:65+Math.floor(this.random()*25), happiness:60+Math.floor(this.random()*30), health:82+Math.floor(this.random()*18), enclosure, medicationNeeded:this.random()<.28, appearance:{...this.pick(APPEARANCES[species])} };
  ...
  tick(amount=1,penSystem=null) { this.animals.forEach(a=>{const installed=penSystem?.penFor(a.species)?.installed||[];const puzzle=installed.some(i=>i.itemId==='puzzle-feeder');const comfort=installed.length>0;a.hunger=Math.max(0,a.hunger-(puzzle?.25:.45)*amount);a.cleanliness=Math.max(0,a.cleanliness-.25*amount);a.happiness=Math.max(0,a.happiness-(comfort?.08:.2)*amount);if(a.hunger<25||a.cleanliness<20)a.health=Math.max(0,a.health-.12*amount);}); }
  care(animal, action) {
